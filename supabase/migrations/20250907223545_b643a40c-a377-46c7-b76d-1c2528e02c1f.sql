...
SELECT 
    ...
    COALESCE(SUM(emi.quantity * emi.price_per_item), 0) as revenue_menu,
    e.labor_cost,
    e.food_cost,
...
LEFT JOIN public.event_menu_items emi ON e.id = emi.event_id
WHERE e.user_id = auth.uid()
GROUP BY e.id, e.event_date, e.number_of_guests, e.labor_cost, e.food_cost, e.total_revenue, e.status, e.title;
...
-- Task history should only be visible to users who own the tasks
